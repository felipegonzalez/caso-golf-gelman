...
ajustes_ensemble <- 
  map(1:100, ~ ajustar_diagnosticos(.x, 
        ...
        params = betas_sim_tbl[.x, ] %>% select(-.chain, -.iteration, -.draw) )) %>% 
  bind_rows()
```

Vemos que todas las corridas no reportaron problemas numéricos o de convergencia:
...
ggplot(sbc_rank_sim, aes(sample = beta)) +
...
No tuvimos problemas en el ajuste. El resumen que4da:
...
g_post 
```
El resultado es bastante malo. Tenemos sobrepredicción para tiros cortos y subpredicciones
...
- Usar splines en la distancia para hacer más flexible el ajuste
- Regresar al análisis conceptual y usar la geometría del problema
