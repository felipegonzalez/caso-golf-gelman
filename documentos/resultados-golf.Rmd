...
No tuvimos problemas numéricos ni de convergencia:
...
```{r}
prior_sd_sigma <- sd(rgamma(1000,15, 500))
...
g_post 
```
El resultado de calibración con posterior es **aceptable**, pero 
tenemos cierta sobrepredicción para tiros largos, lo que indica que nuestro
modelo simple no captura mecanismos que hacen los tiros largos más difíciles:
ya sea porque hay más desviación en el ángulo (no es uniforme), o porque otros
factores como la potencia del tiro intervienen.

### Ideas para siguiente iteración

- Considerar condiciones de éxito para la potencia del tiro.
- Permitir que la desviación aumente conforme la distancia aumenta (?)
